  ...
  const renderTableHeader = () => (
    <TableHeader>
      <TableRow className="bg-slate-50 hover:bg-slate-50 border-b-slate-200">
        ...
        <TableHead className="min-w-[80px] w-[80px] font-semibold text-slate-700">UF</TableHead>
  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      {filteredLeads.length > 0 && !isSearching && (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 px-1 animate-fade-in w-full mt-2">
          <div className="text-sm text-slate-500 font-medium bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">
            Total:{' '}
            <span className="font-bold text-slate-800 text-base">{pagination.totalCount}</span>{' '}
            leads encontrados
          </div>
          <div className="flex items-center gap-2.5 text-xs text-slate-600 bg-white border border-slate-200 px-3 py-2 rounded-lg shadow-sm">
            <span className="font-medium flex items-center gap-1">
              <Search className="w-3.5 h-3.5 text-slate-400 hidden sm:inline" />
              Navegue pelas colunas:
            </span>
            <div className="flex items-center gap-1">
              <kbd
                className="font-sans font-bold text-slate-700 bg-slate-50 border border-slate-300 rounded px-1.5 py-0.5 shadow-sm text-[12px] leading-none flex items-center justify-center h-5 min-w-5 cursor-help"
                title="Seta Esquerda"
              >
                ←
              </kbd>
      ...
      {filteredLeads.length > 0 && pagination.totalPages > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 bg-white border border-slate-200 shadow-sm rounded-xl gap-4">
