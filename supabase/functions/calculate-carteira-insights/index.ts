...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const cnae in cnaeStats) {
        ...
        const safe_ticket = isNaN(ticket_medio_cnae) || !isFinite(ticket_medio_cnae) ? 0 : ticket_medio_cnae;
        const safe_taxa = isNaN(taxa_sucesso) || !isFinite(taxa_sucesso) ? 0 : taxa_sucesso;
        const safe_fit = isNaN(fit_operacional_score) || !isFinite(fit_operacional_score) ? 0 : fit_operacional_score;

        analiseUpserts.push({
            cnae,
            nome_cnae: `Setor: ${stat.segmento}`,
            total_clientes: stat.total_clientes,
            ticket_medio_cnae: Number(safe_ticket.toFixed(2)),
            taxa_sucesso: Number(safe_taxa.toFixed(2)),
            distribuicao_geografica: stat.geo,
            fit_operacional_score: Number(safe_fit.toFixed(2)),
    ...
    for (const seg in segmentStats) {
        ...
        const safeMedia = isNaN(mediaSegmento) || !isFinite(mediaSegmento) ? 0 : mediaSegmento;
        ...
        clustersUpserts.push({
            ...
            oportunidade_score: Number(safeMedia.toFixed(2)),
            prioridade: safeMedia > 5000 ? 'Alta' : (safeMedia > 2000 ? 'Média' : 'Baixa'),
            updated_at: new Date().toISOString()
        })
    }
