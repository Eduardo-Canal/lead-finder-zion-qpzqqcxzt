...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const cnae in cnaeStats) {
      ...
      const safe_ticket =
        isNaN(ticket_medio_cnae) || !isFinite(ticket_medio_cnae) ? 0 : ticket_medio_cnae
      const safe_taxa = isNaN(taxa_sucesso) || !isFinite(taxa_sucesso) ? 0 : taxa_sucesso
      const safe_fit =
        isNaN(fit_operacional_score) || !isFinite(fit_operacional_score) ? 0 : fit_operacional_score
